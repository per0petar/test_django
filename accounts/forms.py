...
class RegisterForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        # Save the provided password in hashed format
        user = super(RegisterForm, self).save(commit=False)  # commit=False returns object but doesn't save it, modify things, then .save() later
        user.set_password(self.cleaned_data["password1"])
        user.active = False
...
class UserLoginForm(forms.Form):
    ...
    def clean(self, *args, **kwargs):
        ...
        if not user:
            raise forms.ValidationError("This user doesn't exist")       # POPRAVI OVO, PASSWORD CHECK NE PROLAZI
